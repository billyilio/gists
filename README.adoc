...
create (:Service{endpoint:'http://mail-v1.staging.api.e-travel.gr', name: 'mailApi'});
...
match (a:Service {name: 'mailApi'}), (b:Application {name: 'panoramix'})
create (a)-[:CALLED_BY]->(b);
...
create (:Service:Rest{name:'mail_to_customers'});
...
create (:Service:Rest{Endpoint:'http://kumquat-v1.stage.api.e-travel.gr/api-v1.0/en/insurance_contracts?email=services@pamediakopes.gr&;password=wtf_flights',name:'kumquat_api_insurance_staging'});
...
match (a:Database{name:'travel_insurances_db'}), (b:ScheduledTask{name:'insurance_cli_old'})
create (a)-[SENDS_TO]->(b);
...
match (a:Database{name:'travel_insurances_db'}), (b:ScheduledTask{name:'insurance_cli_new'})
create (a)-[SENDS_TO]->(b);
